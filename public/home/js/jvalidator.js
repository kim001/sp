    ...
    $(obj).find("[datacol=yes]").each(function () {
        if ($(this).attr("checkexpession") != undefined) {
            switch ($(this).attr("checkexpession")) {
				...
				case "MaxLenStr":
                    {
                        if (!isMaxLenStr($(this).val(), $(this).attr("length"))) {
				...
				case "AjaxPwd":
					{
						if (!isAjaxPwd($(this).attr("url"), $(this).val())) {
				...
				case "isRadio":
					{
						...
						var chk = $('input:radio[name="'+radioVal+'"]:checked').val();
                        if (!isRadio(chk, $(this).attr("eqvalue"))) {
                            Validatemsg = "请选择"+$(this).attr("err") + "！\n";
                            Validateflag = false;
                            ChangeCss($(this), Validatemsg); return false;
                        }
                        break;
				...
				case "isRate":
                    {
                        if (!isRate($(this).val(), $(this).attr("eqvalue"))) {
				...
				case "isMonth":
                    {
                        if (!isMonth($(this).val(), $(this).attr("eqvalue"))) {
                            Validatemsg = $(this).attr("err") + "必须在6~12期之间！\n";
                            Validateflag = false;
                            ChangeCss($(this), Validatemsg); return false;
                        }
                        break;
   ...
   // top.showTopMsg(Validatemsg, 5000, 'error');
    ...
    $(obj).blur(function () {
        if ($(obj).val() != "") {
			...
			else if ($(obj).attr('type') == 'radio') {	//gxunc
                $(obj).removeClass("tooltipinputerr");
            }
			else {
               // $(obj).addClass("select");	gxunc
