...
function showTipsMsg(msg, time, type) {
    if (type == 4) {
        showTopMsg(msg, time, 'success');//头部提示,1、success 2、error 3、warning
    } else if (type == 5) {
        showTopMsg(msg, time, 'error');//头部提示,1、success 2、error 3、warning
    } else if (type == 3) {
        showTopMsg(msg, time, 'warning');//头部提示,1、success 2、error 3、warning
    }
}
...
/*gxunc
function openDialogAjax(url, okUrl, _id, _title, _width, _height, left, top) {
    art.dialog.open(url, {
        id: _id,
        title: _title,
        width: _width,
		...
		$.post(okUrl, {}, function(res){alert(res)});
		return false;
		}
	},
...
function delConfig(url, parm, count) {
    DeleteConfig(url, parm, "注：操作不可恢复，您确定要继续吗？", count);
}
...
function SendConfig(url, parm, count) {
    SendConfig(url, parm, "注：您确定要继续吗？", count);
}
function SendConfig(url, parm, Msg, count) {
    if (count == undefined) {
        count = 1;
    }
    showConfirmMsg(Msg, function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (rs.toLocaleLowerCase() == 'true') {
                    showTipsMsg("成功", 2000, 4);
                    windowload();
                } else if (rs.toLocaleLowerCase() == 'false') {
                    showTipsMsg("失败，请稍后重试", 4000, 5);
                } else {
                    showTipsMsg(rs, 4000, 3);
                }
            });
...
function pqGridResize(obj, lose_height, lose_width) {
    ...
    $(obj).siblings().each(function () {
        if ($(this).hasClass("btnbarcontetn") || $(this).hasClass("tools_bar"))
            height += $(this).height();
    });
    grid_width = grid_width - 2;
    grid_height = grid_height - height - 10;
...
/**表格指定高度
obj：ID，height:-高度,width:-宽度
**/
function pqGridResizeGood(obj, height, width) {
    var grid1 = $(obj).pqGrid({
        width: width,
        height: height
    });
...
function AddTabMenu(tabid, url, name, Isclose, IsReplace) {
    ...
    if (document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        Loading(true);
...
function All_CloseTabMenu() {
    $("#div_tab").find("[Is_Win_close=true]").each(function () {
...
//检查用户是否存在
function IsUserName(UserID, UserName) {
    var IsNum = 1;
    $.post("/Admin/ashx/Public.ashx?action=IsUserName&UserID=" + UserID + "&UserName=" + UserName, function (data) {
        IsNum = data;
    });
    return IsNum;
