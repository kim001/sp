...
class myjson
{
    ...
    function decode($text,$type=1) // 默认type=0返回obj,type=1返回array
    {
        if (empty($text))
        {
            return '';
        ...
        if (function_exists('json_decode'))
        {
			...
			return $this->addslashes_deep_obj(json_decode($text,$type));
        }
	...
	function addslashes_deep($value)
	{
		...
		else
		{
			if(is_string($value))
			{
				//$value = trim(htmlspecialchars($value));//防止被挂马，跨站攻击
				if(!get_magic_quotes_gpc())
				{
						$value = addslashes($value);//防止sql注入
				}
				/*觉得还不安全可以再加上下面2个
			...
			else if(is_array($value))//如果是数组采用递归过滤
			{
				foreach($value as $key => $vl)
				{
					$value[$key] = $this->addslashes_deep($vl);
				}
				return $value;
			}
			else
